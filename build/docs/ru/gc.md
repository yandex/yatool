...
### Очистка телеметрийного кеша Snowden

> **Предупреждение:** Эта опция безвозвратно удаляет неотправленные события телеметрии. Используйте с осторожностью.

По умолчанию `ya gc cache` **не** удаляет кеш телеметрии Snowden (`~/.ya/snowden/`). В standalone-режиме Snowden эта директория содержит chunk-файлы с событиями, которые демон Snowden ещё не успел отправить в backend. Удаление директории приведёт к безвозвратной потере этих событий.

Чтобы явно очистить кеш Snowden, передайте флаг `--clear-snowden`:
```bash
ya gc cache --clear-snowden
```
Используйте этот флаг только когда вы уверены, что потеря неотправленной телеметрии допустима (например, при освобождении места на машине, которую выводят из эксплуатации).
