...
using Events = y_absl::InlinedVector<PollEventHandle*, 5>;
 ...
 public:
  PollEventHandle(int fd, PollPoller* poller)
      : fd_(fd),
        ...
        pollhup_(false),
        watch_mask_(-1),
        shutdown_error_(y_absl::OkStatus()),
  ...
  bool IsOrphaned() const Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    return is_orphaned_;
  }
  void CloseFd() Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    if (!released_ && !closed_) {
      closed_ = true;
      close(fd_);
    }
  }
  bool IsPollhup() const Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) { return pollhup_; }
  void SetPollhup(bool pollhup) Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    pollhup_ = pollhup;
  }
  bool IsWatched(int& watch_mask) const Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    watch_mask = watch_mask_;
    return watch_mask_ != -1;
  }
  bool IsWatched() const Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    return watch_mask_ != -1;
  }
  void SetWatched(int watch_mask) Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    watch_mask_ = watch_mask;
  }
  void OrphanHandle(PosixEngineClosure* on_done, int* release_fd,
                    y_absl::string_view reason) override;
  void ShutdownHandle(y_absl::Status why) override;
  ...
  grpc_core::Mutex* mu() Y_ABSL_LOCK_RETURNED(mu_) { return &mu_; }
...
EventHandle* PollPoller::CreateHandle(int fd, y_absl::string_view /*name*/,
                                      bool track_err) {
...
void PollEventHandle::OrphanHandle(PosixEngineClosure* on_done, int* release_fd,
  ...
  ForceRemoveHandleFromPoller();
  {
    ...
    if (!is_shutdown_) {
      is_shutdown_ = true;
      shutdown_error_ =
          y_absl::Status(y_absl::StatusCode::kInternal, "FD Orphaned");
...
void PollEventHandle::ShutdownHandle(y_absl::Status why) {
...
void PollEventHandle::NotifyOnError(PosixEngineClosure* on_error) {
  on_error->SetStatus(
      y_absl::Status(y_absl::StatusCode::kCancelled,
