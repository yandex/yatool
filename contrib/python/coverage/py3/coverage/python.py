...
def get_python_source(filename: str, force_fs=False) -> str:
    """Return the source code, as unicode."""
    if getattr(sys, 'is_standalone_binary', False) and not force_fs:
        import __res

        if src_code := __res.resfs_read(__res.py_src_key(filename)):
            src_code = src_code.replace(b"\r\n", b"\n").replace(b"\r", b"\n")
            return src_code.decode('utf-8')
        else:
