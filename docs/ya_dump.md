...
Выводит форматированный JSON с информацией о рецептах (тестовых окружениях), используемых в тестах.
...
В отличие от опций, которые модифицируют поведение команды, дополнительные параметры обычно указывают на конкретные объекты действия (например,  модули, директории), над которыми будет выполняться операция.

- `--target=<path>`: Указание конкретного модуля или директории, для которой следует выполнить операцию.
