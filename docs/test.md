...
```
Команда `ta test` поддерживает [параллельный запуск тестов]
...
Такие тесты также описываются в файле `ya.make`. Более подробная информация находится в [документации]().
