...
- Сохранение результатов: Результаты тестов сохраняются в [директорию test-results](#Директория-результатов-тестов).
- Некоторым тестам для корректной работы нужно уметь [взаимодействовать с тестовым окружением](test_contextfile.md).
...
#### Директория результатов тестов

Локально результаты тестов сохраняются в директорию `test-results`. Содержимое этой директории важно для анализа и отладки:

- test-results: Это основная директория, где хранятся результаты тестов. В рабочей копии директория теста создается символьной ссылкой (кроме Windows или при использовании флага `--no-src-links`).
- Внутри директории `test-results` находится поддиректория `<suittype>/testing_out_stuff`, где хранятся логи и другие файлы, порожденные тестами.
- Если тесты завершились неуспешно, в консоль может быть выведена информация о путях к реальным директориям внутри сборочного каталога, где они выполнялись. Эти данные доступны до следующей сборки или запуска тестов, так как сборочные директории не очищаются немедленно. Чтобы сохранить результаты тестов, скопируйте их заранее, поскольку они будут удалены при следующем запуске сборки.
...
- --style: Запуск только style-тестов (включает кэширование).
