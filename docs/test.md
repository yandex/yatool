...
Примечание: Для директории contrib допускается отключение проверки стиля макросом `NO_LINT()`.
...
Отключение проверки: `NO_CHECK_IMPORTS` с указанием списка, например:
...
- Отключение: `NO_LINT(ktlint)` в ya.make файле модуля.
...
- Отключение: `NO_LINT()` в `ya.make` файле модуля.
...
- Множество других опций для настройки окружения и управления тестами (см. “Опции команды ya test”).
...
Более подробная информация находится в [документации](fuzzing.md).
...
Такие тесты также описываются в файле `ya.make`. Более подробная информация находится в [документации](exectest.md).
