...
Более подробный перевод параметров можно изучить [здесь](ya_conf.md)
...
### Возможности переименования `ya.conf`
...
- `ya.${system}.conf` - для системных спецификаций.
- `ya.${command}.conf` - для спецификаций команды.

Модификаторы `${system}` и `${command}` адресуют конфигурационные файлы к определенной системе или команде, например, `ya.make.darwin.conf` для команды make на системе darwin.
...
Внутри одного блока `[[alias]]` можно задавать произвольное количество опций и под-опций.
