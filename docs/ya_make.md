...
1. Синтаксис: Команда печатает справку `ya make  -h, --help`. Используйте `-hh` для большего количества опций и `-hhh` для ещё большего. С описанием всех функций можно ознакомится по [Ссылке](helpfilemake.md)
...
```
- -d, --debug: Сборка в режиме отладки с генерацией отладочной информации.
- -r, --release: Сборка проекта в режиме релиза, что включает оптимизацию исполняемого кода.
- --dist: Включение распределенной сборки для использования сборочных кластеров.
- -v, --verbose: Включение подробного вывода информации о процессе сборки.
- --rebuild: Принудительный пересбор всего проекта, игнорируя кэш предыдущих сборок.
```
