...
Если у вас есть доступ к универсальному менеджеру репозиториев `artifactory`, вы можете использовать его для хранения [загружаемых пакетов](#Загрузка-пакетов-в-artifactory).
...
С помощью команды с ключом `--docker`:
```
...
* По умолчанию пакет будет иметь тег вида `registry.YOURREPO/<repository>/<package name>:<package version>`.
    - `registry.YOURREPO` — реестр образов, задается через параметр `--docker-registry`.
