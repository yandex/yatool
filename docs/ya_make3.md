...
Это команда для автоматизации сборки проектов, включая компиляцию кода, управление зависимостями, запуск тестов и создание исполняемых файлов.

Цель сборки в системе `ya make` — это программа, библиотека или пакет, которые нужно собрать.  
...
Этот файл описывает, что и как будет собрано. 
Результаты сборки по умолчанию размещаются в этой директории как символьные ссылки.
...
- [TARGET]… — цели сборки. Это могут быть конкретные модули, директории с компонентами или специальные аргументы для определенных действий (например, запуск тестов).
При отсутствии цели, `ya make` собирает весь проект, начиная с текущего каталога.

`ya make` с помощщью опций позволяет гибко настраивать параметры сборки:
...
- `ya make -hhh` — для подробного списка опций.
...
Оптимизирует исполняемый код для максимальной производительности, обычно за счет уменьшения размера и удаления отладочной информации.
- `-v`, `--verbose`
...
- `-S=CUSTOM_SOURCE_ROOT`, `--source-root=CUSTOM_SOURCE_ROOT` — путь к проекту, который будет собираться. По умолчанию это текущий рабочий каталог, где запущена команда ya make.
