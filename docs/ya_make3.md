...
Локальная сборка поддерживает режим [контентных UIDов](UIDwork.md#сборка-с-контентными-uid-ами).
...
Процесс сборки в системе `ya make` [описывается](ya_make_yamake.md) в файлах `ya.make`.
Эти файлы находятся рядом с кодом и включают все инструкции, зависимости и параметры для сборки.
Сборка описывается на высокоуровневом полудекларативном языке, состоящем из макросов, их значений, модулей и выражений.
...
`DEPENDS` указывает зависимости на другие проекты, которые нужно собрать и результаты которых должны быть доступны тесту. В параметрах перечисляются относительные пути от корня репозитория.
...
`ya make` предоставляет подробные сообщения об ошибках и предупреждениях на этапе конфигурации (анализ ya.make файлов, построение графа зависимостей2). Эти сообщения могут включать информацию о недостающих файлах, циклических зависимостях, проблемах в конфигурации и многом другом. 
