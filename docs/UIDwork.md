...
Начиная от входов в граф команд (результатов сборки) выполняется такая рекурсивная процедура:
  ...
  Сборка исполняется в специальных сборочных директориях, каждую команду в своей, обеспечивая герметичность процесса.

  2. Доставка результатов: Результаты команд переносятся в отдельные директории сборочного кэша, а также выборочно становятся результатами сборки - символическими ссылками в рабочей копии и файлами в выходной директории.

### Сборка с контентными UID-ами

В режиме сборки с контентными UID-ами (используется в локальной сборке по умолчанию) процесс работы системы сборки претерпевает ряд изменений для улучшения эффективности и уменьшения избыточных перестроений.

#### Принцип работы

Расчёт UID команд: На втором шаге построения графа, система сборки рассчитывает собственный уникальный идентификатор (UID) для каждой команды. Засчитываются всё что и в обычный, кроме UID-ов зависимых команд.

#### Исполнение сборки

Во время исполнения сборка следуют немного изменённому процессу:

1. Проверка кэша: Для UID команды проверяется наличие результата её исполнения в кэше.
2. Повторная проверка зависимости: Если результаты команды не найдены в кэше, аналогичная проверка проводится для всех зависимостей команды.
3. Вычисление хэшей зависимостей: Если результаты зависимых команд пересчитываются, для каждой зависимости рассчитывается хэш результата.
4. Формирование контентного UID: На основе хэшей всех зависимостей и собственного UID команды формируется контентный UID.
5. Проверка кэша по контентному UID: Если для контентного UID имеется закэшированный результат, он извлекается и ассоциируется с обычным UID.
6. Выполнение команды: Если результаты команды не найдены ни по контентному UID, ни по обычному UID, команда выполняется, и её результаты кэшируются как по обычному UID, так и по контентному UID.

#### Преимущества

Основное преимущество использования контентных UID заключается в том, что любые незначительные изменения в зависимостях не приводят к перезапуску всех команд. Это позволяет:
  - Сократить повторное выполнение команд, результаты которых фактически не изменились.
  - Уменьшить общее время сборки за счёт уменьшения количества выполняемых сборочных задач.

#### Ограничения

Несмотря на явные преимущества, данный режим также имеет свои ограничения:
- Бинарная воспроизводимость: Эффективная работа режима контентных UID напрямую зависит от бинарной воспроизводимости результатов команд. Команды, результаты которых содержат непредсказуемые элементы (например, временные метки при выполнении команд, порождающих `.tar`), могут препятствовать полноценному использованию этого режима.
- Единый хэш для всех результатов команды: Хэширование производится для всех результатов команды как единого целого. Если команда генерирует множество результатов, изменение всего одного из них приведёт к пересборке всех зависимых команд, даже если они не используют изменившиеся результаты. Это влияние можно частично смягчить с помощью создания промежуточных узлов, которые передают только необходимые выходные данные другим командам. Такие узлы можно легко перезапустить при изменениях, что гораздо менее затратно, чем перекомпилировать реальные команды.

Отключить режим можно удалением настройки или разово ключом `--no-content-uids` или добавив `content_uids = false` в свой `ya.conf`. 
