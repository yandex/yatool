...
Канонизация в Java позволяет сохранять ожидаемые результаты тестов (канонические данные), чтобы можно было сравнить их с результатами последующих запусков тестов. Используются специальные библиотеки и классы для работы с канонизацией объектов и файлов.

#### Канонизация объектов

Для канонизации объектов в Java используется класс Canonizer из библиотеки devtools/jtest.
...
public class MyTest {
    @Test
    public void testObjectCanonization() {
        MyObject obj = new MyObject();
        // Настроить объект obj
        Canonizer.canonize(obj);
    }
}
...
- local: Если установлено в true, файл будет сохранен локально. По умолчанию файлы загружаются в некое хранилище.

По умолчанию все файлы загружаются в глобальное хранилище, но путем задания параметра local в true, можно сохранять файлы локально. Мы настоятельно не рекомендуем хранить бинарные канонические файлы в репозитории.

Пример канонизации файла:
...
public class MyFileTest {
    @Test
    public void testFileCanonization() {
        String filePath = “path/to/file.txt”;
        // Канонизация файла
        CanonicalFile.canonize(filePath, null, 0, true);
    }
}
