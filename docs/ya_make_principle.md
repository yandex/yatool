...
> Важно: Не запускайте ya make в корневой директории репозитория. Это может привести к попытке собрать весь проект, что может быть непрактичным и затруднительным.
...
> Совет: Чтобы ограничить сборку только текущей целью, используйте ключ -R/--ignore-recurses.
...
3. Построение графа сборочных команд.
