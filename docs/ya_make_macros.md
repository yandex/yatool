...
раньше в `ya.make`, и не видит то, которое будет выставлено ниже по тексту. Однако, если установка переменной меняет свойства модуля или его команд, она может повилять и
...
Добавляет значение к переменной. Существующее значение могло быть определено в глобальной конфигурации сборки, например `SET_APPEND(CFLAGS -fno-omit-frame-pointer)` добавит к глобально определённым для C++ и C модулей флагам тот, что требуется текущему модулю.
...
Задаёт значение по умолчанию для переменной. В отличие от `SET(VarName VarValue)` установит запрошенное значение переменной только если для данной переменной значение ещё нигде не было определено.
...
**Параметры:**
- `STRICT` (режим по умолчанию) управляет интерпретацией `ya.make` при нарушении условий. Если выставлен, то модуль и все варианты [`RECURSE`](#recurse) в его ya.make будет проигнорированы. Это приведёт к тому, что модуль не будет собираться и станет недоступен для зависимостей по [`PEERDIR`](#peerdir) и [`DEPENDS`](../tests/common#depends). Также не станут собираться модули, достижимые только от этого по [`RECURSE`](#recurse), например его тесты.
- `FATAL_ERROR` управляет тем, должна ли выдаваться ошибка или только предупреждение при нарушении условий. Он также безусловно включает `STRICT`-режим интепретации (см.выше).
- `WARNING` не влияет на сборку, только выдаёт configure warning при нарушении условий.
...
Эти макросы по умолчанию работают в режиме `STRICT`. В случае `WARNING` будет лишь выдано предупреждение на консоль, и сборка продолжится. Модуль либо соберётся в запрещённой конфигурации, либо случится ошибка сборки.

В случае с `FATAL_ERROR` или `STRICT` модуль будет исключён из сборки с соответственно configure-ошибкой или предупреждением. `PEERDIR`-зависимости на такой модуль выдадут дополнительные configure-ошибки. Если будут проигнорированы с помощью параметра [`-k` (`--keep-going`)](../../usage/ya_make/#obshie-nastrojki), то возникнут ошибки линковки.
