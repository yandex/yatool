...
* `DIRS dirs` - **[Обязательный]** Список директорий для поиска файлов относительно текущей или `srcdir`. Не должен содержать `${BUILD_ROOT}` и подобных, `..`, `*` или `?`.
...
* `SRCDIR srcdir` - Опциональная базовая директория. Если не задана берется та, где находится `ya.make`.
   ...
   {% note warning %}
    ...
    Использование `srcdir` за пределами текущей директории крайне не рекомендуется, но не запрещено. Значение может содержать `..` или начинаться с  `${BUILD_ROOT}` для адресации от корня Аркадии. Однако `srcdir` не может быть маской и содержать  `*` или `?`.
...
быть указаны с полными путями включая `${BUILD_ROOT}`/`${DIR_BUILD_ROOT}`. Если зависимые файлы не всегда лежат
