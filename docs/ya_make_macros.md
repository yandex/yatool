...
Данный механизм является обёрткой над `RESOURCE_FILES` и позволяет добавлять в ресурсы все файлы с расширением `Ext` из указанных директорий `Dirs`.

Все параметры интерпретируются как пути относительно текущей директории (где находится `ya.make`), но на данный момент может работать некорректно.
...
Этот параметр задает лицензию текущего модуля. Вы можете использовать [`SPDX` выражение для описания лицензии](https://spdx.github.io/spdx-spec/v2.3/SPDX-license-expressions/). Если лицензию не указали явно с помощью макроса `LICENSE`, автоматически применяется лицензия, заданная переменной `DEFAULT_MODULE_LICENSE` в файле конфигурации `ymake.core.conf`.

Каждая лицензия, используемая в `SPDX` выражении, должна быть описана в `ymake.core.conf` с использованием служебных переменных: `LICENSE_PROPERTIES`, `LICENSES_<PROP_NAME>_STATIC`, `LICENSES_<PROP_NAME>_DYNAMIC` или `LICENSES_<PROP_NAME>`. Лицензии без описанных свойств считаются неизвестными и их использование в макросе `LICENSE` вызывает ошибку конфигурации.

Из-за синтаксических ограничений `ya.make`, при использовании скобок в `SPDX` выражении, необходимо заключать выражение или его часть в кавычки.
...
Макрос устанавливает ограничения на лицензии библиотек, которые текущий модуль использует напрямую или косвенно. Если один и тот же макрос применяется несколько раз к одному модулю, все его вызовы должны использовать идентичный тип ограничения (либо все `ALLOW_ONLY`, либо все `DENY`).

* `ALLOW_ONLY` предполагает, что каждая зависимая библиотека должна иметь хотя бы одну лицензию с разрешёнными свойствами.
* `DENY` требует, чтобы каждая зависимая библиотека имела хотя бы одну лицензию без запрещённых свойств.

Свойства лицензий задаются в виде строк без пробелов и перечисляются в переменной `LICENSE_PROPERTIES` в файле `ymake.core.conf`.

Лицензии могут налагать различные ограничения на проекты, которые связываются с библиотекой статически или динамически.
При описании свойств лицензий указывается, какие из них применяются к проектам со статической линковкой, а какие ограничивают только тех, кто осуществляет динамическую линковку.
Ограничения, указанные в макросе `LICENSE_RESTRICTION`, автоматически учитывают тип линковки с используемой библиотекой.

Для различных языков программирования статическая и динамическая линковка понимается следующим образом:

* Языки, компилируемые в нативный код (`C++`, `Go`): стандартный способ
* `Python`: так как Python-код упаковывается в ресурсы автономного бинарного файла, все зависимости между модулями через `PEERDIR` считаются статической линковкой.
* `JVM` языки (`Jav`a, `Kotlin`): зависимости между библиотеками через `PEERDIR` считаются динамической линковкой, аналогично толкованию зависимости через `classpath` в отношении [`LGPL` по `FSF`](https://www.gnu.org/licenses/lgpl-java.en.html). Зависимость от кода `JNI` считается динамической линковкой.

В некоторых случаях соблюдение требований лицензии может быть обеспечено без внесения изменений в систему сборки.
Для таких случаев предусмотрен список исключений, задаваемый макросом `LICENSE_RESTRICTION_EXCEPTIONS`.
Исключения могут включать только полный путь к библиотеке, использование шаблонов путей не допускается.
Этот список исключений должен использоваться только в согласованных с юристами случаях.
...
Макрос позволяет задать требование, чтобы среди прямых и косвенных зависимостей модуля не было нежелательных библиотек или сборочных инструментов. Для одного модуля можно использовать либо черный список через `CHECK_DEPENDENT_DIRS(DENY ...)`, либо белый список через `CHECK_DEPENDENT_DIRS(ALLOW_ONLY ...)`. Если макрос используется несколько раз в пределах одного модуля, тип ограничения должен быть одинаковым во всех вызовах (либо везде `DENY`, либо везде `ALLOW_ONLY`). Нарушения этого правила считаются ошибками конфигурации. Все ограничения от вызовов макроса суммируются.

Ограничения можно задавать двумя способами. Первый способ — указание пути в репозитории, в этом случае все модули в поддиректориях этого пути подпадают под ограничения. Второй способ — использование `Ant`-подобного шаблона пути посредством ключевого слова `GLOB`. В этом случае ограничения действуют только на пути, полностью соответствующие шаблону, но не на их поддиректории. Например: `CHECK_DEPENDENT_DIRS(DENY some/nested/path/bad)` запретит зависимости как от `some/nested/path/bad`, так и от `some/nested/path/bad/lib`, в то время как `CHECK_DEPENDENT_DIRS(DENY GLOB **/nes?ed/*/bad)` запретит зависимости от `some/nested/path/bad`, но не от `some/nested/path/bad/lib`. Ключевое слово `GLOB` действует только на следующий за ним аргумент.

По умолчанию ограничения касаются как зависимостей типа `PEERDIR`, так и зависимостей типа `TOOL`. Однако поведение ограничений можно изменить с помощью ключевых слов `PEERDIRS` и `ALL`. Эти ключевые слова влияют на все последующие ограничения.
...
При использовании `DENY` можно указать исключения, применив ключевое слово `EXCEPT`, которое воздействует на следующее за ним ограничение, преобразуя его в правило исключения для чёрного списка.
...
Макрос позволяет выявлять конфликты между реализациями одного и того же функционала, предоставляемыми разными библиотеками.

Библиотеки, которые могут конфликтовать (или разные версии одной и той же библиотеки), маркируются как реализации определённого функционала (или нескольких функционалов, если необходимо). Во время конфигурации конечных целей (программ, динамических библиотек) проверяется, что в транзитивной зависимости нет двух библиотек, заявляющих себя провайдерами одного и того же функционала. Если проверка не проходит, возникает ошибка конфигурации, и в диагностическом сообщении указываются пути по зависимостям до каждой из конфликтующих библиотек.
...
Макрос предназначен для указания конкретного значения свойства в модуле, например, версии.

Макрос может использоваться многократно для указания различных свойств.
...
```
Будет сгенерирована ошибка конфигурации типа `Misconfiguration`. Этот функционал предназначен для объявления свойств конкретных модулей, и перезапись свойства внутри того же модуля не имеет смысла, поэтому это считается ошибкой.

### CHECK_FEATURE_VERSION_CONFLICTS

Используется исключительно в сочетании с `FEATURE_VERSION` для проверки соответствия свойств всех подчиненных модулей свойствам текущего модуля.
...
FEATURE_VERSION(feature2 ver2.5)
```
Это означает, что требуется проверка всех зависимостей текущего проекта (как прямых, так и транзитивных) на соответствие свойствам `feature1` и `feature2`, которые должны иметь строго идентичные значения.

Если хотя бы одно из свойств не соответствует настройкам проекта, будет сгенерирована ошибка конфигурирования `BadDep`.

#### Пример использования без FEATURE_VERSION
...
```
Возникает ошибка конфигурирования `Misconfiguration`, поскольку отсутствует объект для проверки, что делает данную конфигурацию бессмысленной.
...
Макрос обеспечивает контроль того, что в наборе всех зависимостей модуля (включая транзитивные) присутствуют заданные зависимости. `Paths...` указывает список директорий, в которых описаны сборки модулей, наличие которых проверяется.

Этот макрос может быть полезен для дополнительного контроля в файлах для `INCLUDE`. Например, если файл описывает общую часть сборки нескольких плагинов для одной системы, макрос `REQUIRES` может использоваться для проверки наличия зависимости от общего модуля, определяющего интерфейс плагинов. Такой модуль может зависеть от кода реализации (например, в `LIBRARY`), от которого, в свою очередь, зависит финальный модуль самого плагина (например, `DLL`).
...
Вызов макроса `EXCLUDE_TAGS` позволяет отключить инстанциацию подмодулей **мультимодуля** для конкретных вариантов (тегов), указанных в аргументах макроса. По умолчанию инстанцируются все варианты мультимодуля, за исключением тех, у которых свойство `.INCLUDE_TAG` установлено в значение `no`.

## INCLUDE_TAGS
```INCLUDE_TAGS(Tags)```

Вызов макроса `INCLUDE_TAGS` позволяет включить инстанциацию подмодулей **мультимодуля** для конкретных вариантов (тегов), указанных в аргументах макроса.

### ONLY_TAGS
```ONLY_TAGS(Tags)```

Вызов макроса `ONLY_TAGS` изменяет набор инстанциаций подмодулей **мультимодуля**, оставляя только указанные варианты (теги, перечисленные в аргументах макроса).
