...
Управление кэшем сборки критически важно для обеспечения эффективности, экономии дискового пространства и предотвращения потенциальных проблем с производительностью в процессе разработки.
...
Конфигурация очистки кэшей может быть настроена через файлы конфигурации: локально на машине пользователя в файле `~/.ya/ya.conf` или в репозитории проекта в файле `junk/<login>/ya.conf`. Рекомендуется использовать файл конфигурации в репозитории, чтобы обеспечить согласованность настроек с конкретным проектом

### Основные функции:

1. Полная очистка вспомогательных файлов - удаление всех инструментальных файлов и прочих необходимых для сборки данных, которые в нормальном режиме удаляются автоматически.
2. Удаление временных файлов - за исключением локального кэша сборки, который очищается согласно определенным критериям.
...
Очистка данных из кэша результатов выполняется автоматически по истечении установленного срока действия, стандартно равного одной неделе. Срок действия файлов в кэше можно скорректировать через параметр symlinks_ttl.
