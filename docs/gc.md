...
#### Результаты сборки 
Результаты сборки могут быть сложены на постоянное хранение в место, указанное с помощью опции `-o/--output <path>`. Система сборки никак не отслеживает файлы в директории `<path>`, и удалять их надо вручную.
