...
```
  ...
  Прогресс     Конфигурация и платформа Вход или результат команды              Количество Занято 
                                                                               команд     место в кэше
```
[Конфигурирование]() имеет собственный индикатор прогресса:
```
...
#### Настройки Локального Кэширования
...
- Строгий контроль зависимостей:
  Обеспечение строгого контроля и проверки зависимости модулей. Эта настройка гарантирует, что все зависимости модуля будут корректно установлены и проверены перед началом сборки. Это помогает избежать проблем, связанных с отсутствующими или несовместимыми зависимостями, и обеспечивают стабильность сборки.

- Локальное кэширование:
  Настройка кэша для локальных сборок, чтобы ускорить процесс при повторных запусках. Кэширование результатов сборки помогает существенно сократить время на сборку, так как уже обработанные результаты используются повторно. Локальное кэширование может быть настроено так, чтобы сохранять промежуточные и финальные результаты.

- Локальное исполнение сборки:
  Конфигурация и параметры для выполнения сборки на локальной машине. Локальная сборка обычно более быстрая и прямолинейная, так как она не использует удаленные ресурсы. Вы можете настроить параметры локального исполнения так, чтобы максимально эффективно использовать ресурсы вашей машины, включая процессорное время, память и дисковое пространство.
