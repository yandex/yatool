...
Настройки исполнения сборки включают следующие параметры:
  ...
  - Локальное кэширование: Настройка кэша для локальных сборок, чтобы ускорить процесс при повторных запусках.
...
По-молчанию локальная сборка и автосборка исполняются в режиме, где каждому сборочному узлу на чтение доступен весь репозиторий. Контроль зависимостей возложен на стадию конфигурирования: она анализирует зависимости команд и засчитывает все нужные файлы в UID-ы, более подробно можно прочитать здесь.
