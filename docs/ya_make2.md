...
При невыполнении тестов на консоль также могут быть выданы пути из реальных директорий, в директории сборки, где тесты выполнялись. Чтобы эти данные были доступны после работы тестов,сбо рочные директории где исполняются тесты чистятся не сразу, а при следующей сборке. Т.е. данные в такой директории доступны только до следующей сборки/запуска тестов.
