...
Команда пакетирования, позволяет собирать различные типы пакетов, описанных в специальных json-файлах, выкладывать и публиковать в пакетных репозиториях.

Команда позволяет пакетировать код, собранный системой сборки ya make в различных фиксированных конфигурациях.
...
Собрать пакет по json-описанию.

`ya package [OPTION]... [PACKAGE DESCRIPTION FILE NAME(S)]...`

[Подробная документация по ya package в кластере системы сборки](https://docs.yandex-team.ru/ya-make/usage/ya_package/)

## Опции

{% cut "Опции пакетирования" %}
```
    --no-cleanup        Do not clean the temporary directory
    --change-log=CHANGE_LOG
                        Change log text or path to the existing changelog file
...
```
  YT cache
    ...
    --yt-store          Use YT storage
    --yt-store-threads=YT_STORE_THREADS
                        YT store max threads (default: 16)
    Expert options
    --yt-token-path=YT_TOKEN_PATH
                        YT token path (default: /home/vpozdyayev/.yt/token)
    Internal options
    --yt-proxy=YT_PROXY YT storage proxy (default: hahn.yt.yandex.net)
    --yt-dir=YT_DIR     YT storage cypress directory pass (default: //home/devtools/cache)
