...
def resolve_java_srcs(
    ...
    for vis, hid, patterns in (
        ...
        for pattern in patterns:
            if (pattern if pattern.find('/') == -1 else pattern.rsplit('/', 1)[1]).startswith('.'):
                ...
                vis.append(pattern)
        re_patterns = list(map(pattern_to_regexp, vis + hid))
    ...
    for inc_patterns, exc_patterns, with_hidden_files in (
        ...
        for f in list_all_files(srcdir, hidden_files=with_hidden_files):
            # print f, [i.pattern for i in inc_patterns], [i.pattern for i in exc_patterns]
            excluded = False
...
def do_it(
    ...
    j, r, k, g = resolve_java_srcs(directory, include_patterns, exclude_patterns, False, resolve_kotlin, resolve_groovy)
    open(sources_file, 'w').write(' '.join(j))
    open(resources_file, 'w').write(' '.join(r))
    if kotlin_sources_file:
        open(kotlin_sources_file, 'w').write(' '.join(k + j))
    if groovy_sources_file:
        open(groovy_sources_file, 'w').write(' '.join(g + j))
