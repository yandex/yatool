...
def parse_args():
    ...
    parser.add_argument("--config", help="Relative path to vitest config, from testing module root")
...
def run_tests(opts):
    vitest_stdout = os.path.join(opts.output_dir, "vitest.out")
    vitest_stderr = os.path.join(opts.output_dir, "vitest.err")
    vitest_results_file = os.path.join(opts.output_dir, "vitest_results.json")

    cmd = get_test_cmd(opts, vitest_results_file)
    ...
    try:
        res = shared.tee_execute(cmd, vitest_stdout, vitest_stderr, strip_ansi_codes=False, on_timeout=shutdown)
        exit_code = res.returncode
    except process.SignalInterruptionError:
        exit_code = const.TestRunExitCode.TimeOut

    with open(vitest_results_file, "r") as f:
        suite = make_suite(opts, f.read())
        shared.dump_trace_file(suite, opts.tracefile)

    return exit_code


def get_test_cmd(opts, results_file):
    cmd = [
        os.path.join(opts.nodejs, "node"),
        os.path.join(opts.node_path, ".bin", "vitest"),
        "run",
        "--config",
...
def make_suite(opts, json_report):
    ...
    for test in tests:
        rel_test_path = os.path.relpath(test.file, opts.test_for_path)
        if len(test.suites) == 0:
            full_test_name = "{}::{}".format(rel_test_path, test.name)
        else:
