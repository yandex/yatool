...
class CoverageExtractorTestSuite(common_types.AbstractTestSuite):
    ...
    def get_run_cmd(self, opts, retry=None, for_dist_build=True):
        ...
        return tools.get_test_tool_cmd(opts, 'run_coverage_extractor', self.global_resources, wrapper=True, run_on_target_platform=True) + [
            '--binary', self.binary_path('$(BUILD_ROOT)'),
            '--tracefile', os.path.join(test_work_dir, const.TRACE_FILE_NAME),
