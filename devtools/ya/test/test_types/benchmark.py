...
class GBenchmarkSuite(AbstractBenchmarkSuite):
    ...
    def get_run_cmd(self, opts, retry=None, for_dist_build=True):
        ...
        cmd = test.util.tools.get_test_tool_cmd(opts, 'run_g_benchmark', self.global_resources, wrapper=True, run_on_target_platform=True) + [
...
class YBenchmarkSuite(AbstractBenchmarkSuite):
    ...
    def get_run_cmd(self, opts, retry=None, for_dist_build=True):
        ...
        cmd = test.util.tools.get_test_tool_cmd(opts, 'run_y_benchmark', self.global_resources, wrapper=True, run_on_target_platform=True) + [
            '--binary', self.binary_path('$(BUILD_ROOT)'),
            '--tracefile', os.path.join(test_work_dir, test.const.TRACE_FILE_NAME),
            '--output-dir', os.path.join(test_work_dir, test.const.TESTING_OUT_DIR_NAME),
            '--project-path', self.project_path,
