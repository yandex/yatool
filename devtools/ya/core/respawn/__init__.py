...
def _src_root_respawn(arc_dir, handler_python_major_version=None):
    ...
    reason_py3, expected_ya_bin_python_version, ya_script_python_force = check_py23_respawn_reason(
...
def check_py23_respawn_reason(
    handler_pyver, _current_ya_bin_version=EmptyValue, _ya_major_version=EmptyValue, _ya_version_force=EmptyValue
):
        ...
        if ya_pyver_require == 23:
            ...
            if ya_bin_version != handler_pyver:
                reason = "ya{}handler{}{}".format(
                    ya_bin_version,
                    handler_pyver,
                    '_force' if ya_pyver_set_forced else '',
                )
    ...
    if ya_bin_version != handler_pyver:
        logger_pyver.info(
            "Incompatible ya-bin and handler python major version: `{}` vs `{}`".format(ya_bin_version, handler_pyver)
        )
