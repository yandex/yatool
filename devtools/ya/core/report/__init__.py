...
from devtools.ya.core import report_common
from devtools.ya.core.report_common import (  # noqa
    default_namespace,
    get_distribution,
    ReportEncoder,
    system_info,
...
class CompositeTelemetry:
    ...
    def report(self, key, value, namespace=default_namespace(), urgent=False):
        if self.no_backends:
            return
        svalue = report_common.sanitize_value(value, SUPPRESSIONS)
        ...
        for telemetry_name, telemetry in self.iter_backends():
            telemetry.push(report_common.create_event(key, svalue, namespace), urgent=urgent)
