...
def _gen(
    ...
    with conf_ctx as conf_dir:
        ...
        res, evlog_dump = devtools.ya.build.ymake2.ymake_dump(
            ...
            managed_dep_tree_json=managed_dep_tree_json,
...
def gen_managed_dep_tree(
    ...
    return _gen(
        ...
        build_type=build_type,
        build_targets=build_targets,
        # ymake prints the tree to stdout, so everything else has to go to stderr (--xE)
        debug_options=(debug_options + ['E']) if as_json else debug_options,
        flags=flags,
        managed_dep_tree=build_targets,
        managed_dep_tree_json=as_json,
        ymake_bin=ymake_bin,
        host_platform=host_platform,
        target_platforms=target_platforms,
        evlog=True,
