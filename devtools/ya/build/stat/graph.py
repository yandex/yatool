...
def create_graph_with_ymake_log(graph, local_log_as_json):
    if not isinstance(graph, Graph):
        graph = Graph(graph)
        graph.log_json = local_log_as_json

    abstract_tasks_by_output = {}
    for abstract_task in graph.abstract_tasks.values():
        if abstract_task.meta is not None:
            for output in abstract_task.meta['outputs']:
                abstract_tasks_by_output[output] = abstract_task

    end_events_by_name = {}
    start_events_by_name = {}
    for log_item in local_log_as_json:
        time = int(log_item['_timestamp']) // 1000
        event_type = log_item['_typename']

        if event_type == 'NEvent.TTaskStart':
            name = log_item['Name'].replace('$B', '$(BUILD_ROOT)')
            if name in abstract_tasks_by_output:
                uid = abstract_tasks_by_output[name].uid

                run_task = graph.get_run_task(uid, LOCAL_HOST)
                run_task.abstract = abstract_tasks_by_output[name]
                run_task.start_time = time

                if name not in start_events_by_name:
                    start_events_by_name[name] = []
                start_events_by_name[name].append(log_item)

        elif event_type == 'NEvent.TTaskSuccess':
            name = log_item['Name'].replace('$B', '$(BUILD_ROOT)')
            if name in abstract_tasks_by_output:
                uid = abstract_tasks_by_output[name].uid
                ...
                run_task = graph.get_run_task(uid, LOCAL_HOST)
                run_task.abstract = abstract_tasks_by_output[name]
                run_task.end_time = time

                if name not in end_events_by_name:
                    end_events_by_name[name] = []
                end_events_by_name[name].append(log_item)

        elif event_type == 'NEvent.TTaskFailure':
            name = log_item['Name'].replace('$B', '$(BUILD_ROOT)')
            if name in abstract_tasks_by_output:
                uid = abstract_tasks_by_output[name].uid
                ...
                run_task = graph.get_run_task(uid, LOCAL_HOST)
                run_task.abstract = abstract_tasks_by_output[name]
                run_task.end_time = None

                if name not in end_events_by_name:
                    end_events_by_name[name] = []
                end_events_by_name[name].append(log_item)

        elif event_type == 'NEvent.TTaskInducedFailure':
            name = log_item['Name'].replace('$B', '$(BUILD_ROOT)')
            if name in abstract_tasks_by_output:
                uid = abstract_tasks_by_output[name].uid
        ...
        elif event_type == 'NEvent.TTaskFromCache':
            name = log_item['Name'].replace('$B', '$(BUILD_ROOT)')
            if name in abstract_tasks_by_output:
                uid = abstract_tasks_by_output[name].uid
    ...
    for output in abstract_tasks_by_output:
        start_events = 0
        if output in start_events_by_name:
            start_events = len(start_events_by_name[output])
            if start_events > 1:
                logger.warn('found more than one start build event by name [%s].' % start_events_by_name[output])

        end_events = 0
        if output in end_events_by_name:
            end_events = len(end_events_by_name[output])
            if end_events > 1:
                logger.warn('found more than one end build event by name [%s].' % end_events_by_name[output])

        if start_events != end_events:
            logger.warn(
                'start events count differs from end events count [name=%s, starts=%d, ends=%d].'
                % (output, start_events, end_events)
            )
    ...
    logger.info('Node count in the dependency graph is %d.' % (len(graph.resource_nodes) - 1))
    logger.info('Dependency count in the graph is %d.' % dependency_count)
