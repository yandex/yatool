...
from yalibrary.ya_helper.common import GSIDParts, sorted_items
...
class YaBaseOptions(BaseOptions):
    def __init__(self, **kwargs):
        super(YaBaseOptions, self).__init__(**kwargs)
        self._handler = None
...
class YaCommandOptions(YaBaseOptions):
    def __init__(self, handler, *cmd, **kwargs):
        super(YaCommandOptions, self).__init__(**kwargs)
    ...
    def _choose_class(cls, other_class):
        ...
        return super(YaCommandOptions, cls)._choose_class(cls)
...
class YaMakeOptions(YaBaseOptions):
    def __init__(self, **kwargs):
        ...
        if self.gsid is not None:
            if isinstance(self.gsid, six.string_types):
    ...
    def _generate_post_handler(self):
        ...
        if self.test_size_timeout:
            ...
            result += [
                '{size}={timeout}'.format(size=key, timeout=value)
                for key, value in six.iteritems(self.test_size_timeout)
            ]
    ...
    def _generate_env(self):
        env = super(YaMakeOptions, self)._generate_env()
        if self.dist_priority:
...
class YaOnlyAllowedOptions(YaMakeOptions):
    ...
    def __init__(self, **kwargs):
        ...
        super(YaOnlyAllowedOptions, self).__init__(**kwargs)
    ...
    def _choose_class(cls, other_class):
        ...
        return super(YaOnlyAllowedOptions, cls)._choose_class(other_class)
