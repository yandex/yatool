...
class BaseSupport(object):
    ...
    def is_marker(self, marker):
        if marker and (marker in self._markers or marker.startswith("c:")):
            return True
...
def transform(text, transformer, is_marker, iter_regex):
    ...
    res.append(transformer(marker, substr))
    # Calling it with empty params to make sure that all the accumulated markers are applied
    # This is important when markers are set in the end of the text
    res.append(transformer(None, ''))
    return ''.join(res)
