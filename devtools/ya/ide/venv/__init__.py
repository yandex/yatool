...
class VenvOptions(core.yarg.Options):
    OPT_GROUP = core.yarg.Group('Python venv options', -1)
    ...
    def consumer():
        return [
            core.yarg.ArgConsumer(
                ...
                hook=core.yarg.SetValueHook('venv_root'),
                group=VenvOptions.OPT_GROUP,
            ),
            core.yarg.ArgConsumer(
                ['--venv-with-pip'],
                help='Install pip to venv',
                hook=core.yarg.SetConstValueHook('venv_with_pip', True),
                group=VenvOptions.OPT_GROUP,
            ),
            core.yarg.ArgConsumer(
                ['--venv-add-tests'],
                help='Include tests',
                hook=core.yarg.SetConstValueHook('venv_add_tests', True),
                group=VenvOptions.OPT_GROUP,
            ),
            core.yarg.ArgConsumer(
                ['--venv-tmp-project'],
                help='Temporary project path',
                hook=core.yarg.SetValueHook('venv_tmp_project'),
                group=VenvOptions.OPT_GROUP,
            ),
            core.yarg.ArgConsumer(
    ...
    def postprocess(self):
        if self.venv_root is None:
            raise core.yarg.ArgsValidatingException('Virtual environment root (--venv-root) must be specified.')
