    ...
    for name, module in run_modules.items():
        ...
        if module_type in consts.PROGRAM_MODULE_TYPES:
            ...
            tasks.append(OrderedDict((
                ...
                ("detail", module["path"]),
                ("type", "shell"),
                ("command", "%s test --run-all-tests --regular-tests %s %s" % (ya_bin_path, module_args, abs_module_path)),
                ("group", "test"),
            )))
            if with_prepare:
                tasks.append(OrderedDict((
                    ...
                    ("detail", module["path"]),
                    ("type", "shell"),
                    ("command", "%s test --run-all-tests --regular-tests --keep-going --test-prepare --keep-temps %s %s" % (ya_bin_path, module_args, abs_module_path)),
                    ("group", "build"),
                )))
    return tasks
    ...
    if "CPP" in languages:
        ...
        tasks.append(
            OrderedDict((
                ("label", "<Codegen for C++>"),
                ("type", "shell"),
                ("command", consts.CODEGEN_CPP_TASK.format(args=CODEGEN_ARGS, targets=TARGETS, ya_path=ya_bin_path, output_dir=codegen_cpp_dir)),
                ("group", "build"),
            )),
        )
