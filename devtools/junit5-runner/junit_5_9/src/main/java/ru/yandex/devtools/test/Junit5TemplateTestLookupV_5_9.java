...
/**
 * Discovers parameterized invocations of template test methods.
 * Covers JUnit Jupiter 5.9.x-5.11.x: DynamicDescendantFilter.allow was renamed
 * to allowUniqueIdPrefix in 5.9.0, while the 5.12+ constructor signatures
 * (OutputDirectoryProvider, enclosingInstanceTypes) have not appeared yet.
 */
public class Junit5TemplateTestLookupV_5_9 implements Junit5TemplateTestLookup {
    ...
    private Junit5TemplateTestLookupV_5_9(CachedTestNames<String, TestIdentifier> testName, LauncherDiscoveryRequest request) {
        ...
        // No more than context initialization
    ...
    public void discoverTemplateInvocation(TestIdentifier test, MethodSource methodSource,
        ...
        try {
            // Look up the matching test method by its description
            ...
            if (method != null) {
                TestTemplateYaTestDescriptorV_5_9 descriptor = new TestTemplateYaTestDescriptorV_5_9(
    ...
    // Returns a lazily initialized lookup - it is not initialized unless
    // parameterized or repeated tests are encountered
    static Supplier<Junit5TemplateTestLookup> lazyLookup(CachedTestNames<String, TestIdentifier> testName, LauncherDiscoveryRequest request, Path ignore) {
        return new Supplier<Junit5TemplateTestLookup>() {
            private Junit5TemplateTestLookup lookup;
            @Override
            public Junit5TemplateTestLookup get() {
                if (lookup == null) {
                    lookup = new Junit5TemplateTestLookupV_5_9(testName, request);
                }
                return lookup;
            }
        };
