...
namespace {
    ...
    struct TGlobPatternInfo {
        ...
        TUniqVector<TCmdElemId> VarElemIds;
        TUniqVector<TCmdElemId> Excludes;
        ...
        for (const auto& edge : node.Edges()) {
            if (edge.Value() == EDT_Property && edge.To()->NodeType == EMNT_Property) {
                ...
                if (propName == NProps::GLOB_HASH) {
                    ...
                    res.VarElemIds.Push(AssumeCmd(edge.To()->ElemId));
                } else if (propName == NProps::GLOB_EXCLUDE) {
                    if (res.Excludes.Push(AssumeCmd(edge.To()->ElemId))) {
                ...
                res.WatchDirs.Push(AssumeFile(edge.To()->ElemId));
            }
        }
        return res;
    }
    ...
    void UpdateGlobNode(TUpdIter& updIter, TDGIterAddable& st, TStringBuf pattern, TFileView dirName, TModule* module) {
        ...
        try {
            ...
            TUniqVector<TFileElemId> matchIds;
    ...
    inline TAddDepContext::TPropsSearchFunction StatePropertiesSearchFunction(const TPropertiesState& props, const TUsingRules& restrictedProps) {
        return [&props, &restrictedProps](TPropertyType type) -> const TVector<TDepsCacheId>* {
            if (restrictedProps.Defined() && restrictedProps->get().contains(type)) {
                return nullptr;
            }
            auto values = props.FindValues(type);
            return values ? &values->Data() : nullptr;
        };
    }

    inline TAddDepContext::TPropsSearchFunction CachedPropertiesSearchFunction(const TRawIncludesInfo& rawIncludesInfo) {
        return [&rawIncludesInfo](TPropertyType type) -> const TVector<TDepsCacheId>* {
            if (auto ptr = rawIncludesInfo.FindPtr(type); ptr && !ptr->empty()) {
                return &ptr->Data();
            }
            return nullptr;
        };
...
inline void TDGIterAddable::StartEdit(TYMake& yMake, TUpdIter& dgIter) {
    ...
    if (Y_UNLIKELY(reassemble && Node.NodeType == EMNT_NonParsedFile && !TFileConf::IsLink(AssumeFile(Node.ElemId)) &&
        ...
        const TModAddData* modData = dgIter.GetAddedModuleInfo(MakeDepFileCacheId(AssumeFile(Node.ElemId)));
    ...
    switch (Node.NodeType) {
        case EMNT_NonParsedFile:
            if (Add && !Add->NeedInit2 && Dep.DepType != EDT_Include) {
                ...
                context.InduceDeps(StatePropertiesSearchFunction(props, restrictedProps));
        ...
        if (isModule) {
            ...
            if (!Graph.GetFileName(st.Node).IsLink()) {
                if (const auto* module = YMake.Modules.Get(AssumeFile(state[st.ModulePosition].Node.ElemId)); module && module->IsLoaded()) {
                    if (!module->GetOwnEntries().has(AssumeFile(st.Node.ElemId)) && !module->GetSharedEntries().has(AssumeFile(st.Node.ElemId))) {
...
inline TUpdIter::EDepVerdict TUpdIter::AcceptDep(TState& state) {
    ...
    if (IsModuleType(st.Node.NodeType)) {
        auto* module = YMake.Modules.Get(AssumeFile(st.Node.ElemId));
        Y_ASSERT(module);
        auto moduleInfo = GetAddedModuleInfo(MakeDepFileCacheId(AssumeFile(st.Node.ElemId)));
        Y_ASSERT(moduleInfo);
        if (module->IsLoaded()) {
    ...
    if (IsSearchDirDep(st.Node.NodeType, dep.DepType, dep.DepNode.NodeType)) {
        ...
        DelayedSearchDirDeps.GetNodeDepsByType(st.Node, EDT_Search).Push(AssumeFile(dep.DepNode.ElemId));
        return EDepVerdict::No;
    }
