...
static inline bool is_subinterpreter() {
    return PyInterpreterState_Main() != PyInterpreterState_Get();
}

typedef struct {
    PyObject* decimal_type = NULL;
    PyObject* timezone_type = NULL;
    PyObject* timezone_utc = NULL;
    PyObject* uuid_type = NULL;
    PyObject* validation_error = NULL;
    PyObject* decode_error = NULL;

    PyTypeObject* decoder_type = NULL;
    PyTypeObject* encoder_type = NULL;
    PyTypeObject* validator_type = NULL;
    PyTypeObject* rawjson_type = NULL;

    PyObject* astimezone_name = NULL;
    PyObject* hex_name = NULL;
    PyObject* timestamp_name = NULL;
    PyObject* total_seconds_name = NULL;
    PyObject* utcoffset_name = NULL;
    PyObject* is_infinite_name = NULL;
    PyObject* is_nan_name = NULL;
    PyObject* start_object_name = NULL;
    PyObject* end_object_name = NULL;
    PyObject* default_name = NULL;
    PyObject* end_array_name = NULL;
    PyObject* string_name = NULL;
    PyObject* read_name = NULL;
    PyObject* write_name = NULL;
    PyObject* encoding_name = NULL;

    PyObject* minus_inf_string_value = NULL;
    PyObject* nan_string_value = NULL;
    PyObject* plus_inf_string_value = NULL;
} rapidjson_state;

static inline rapidjson_state*
get_rapidjson_state(PyObject* module)
{
    void* state = PyModule_GetState(module);
    assert(state != NULL);
    return (rapidjson_state*)state;
}


static rapidjson_state*
find_rapidjson_state_by_def(PyTypeObject* type);
...
static PyObject* do_decode(rapidjson_state* state,
                           PyObject* decoder,
...
static PyObject* do_encode(rapidjson_state* state, PyObject* value, PyObject* defaultFn, bool ensureAscii,
...
static PyObject* do_stream_encode(rapidjson_state* state, PyObject* value, PyObject* stream, size_t chunkSize,
...
public:
    ...
    PyReadStreamWrapper(rapidjson_state* state, PyObject* stream, size_t size)
        : moduleState(state),
        stream(stream) {
        Py_INCREF(stream);
...
private:
    void Read() {
        ...
        chunk = PyObject_CallMethodObjArgs(stream, moduleState->read_name, chunkSize, NULL);
...
public:
    ...
    PyWriteStreamWrapper(rapidjson_state* state, PyObject* stream, size_t size)
        : moduleState(state), stream(stream) {
        ...
        cursor = buffer;
        multiByteChar = NULL;
        isBinary = !PyObject_HasAttr(stream, moduleState->encoding_name);
    }
    ...
    void Flush() {
        ...
        if (c == NULL) {
            ...
            PyObject* res = PyObject_CallMethodObjArgs(stream, moduleState->write_name, c, NULL);
            if (res == NULL) {
                // Likewise
            } else {
                Py_DECREF(res);
...
static PyType_Slot RawJSONType_Slots[] = {
    {Py_tp_dealloc, (void*)RawJSON_dealloc},
    {Py_tp_doc, (void*)rawjson_doc},
    {Py_tp_members, RawJSON_members},
    {Py_tp_new, (void*)RawJSON_new},
    {0, 0},
};


static PyType_Spec RawJSONType_Spec = {
    .name = "rapidjson.RawJSON",
    .basicsize = sizeof(RawJSON),
    .flags = Py_TPFLAGS_DEFAULT,
    .slots = RawJSONType_Slots,
};
...
accept_datetime_mode_arg(PyObject* arg, unsigned &datetime_mode)
{
    if (arg != NULL && arg != Py_None) {
        if (is_subinterpreter()) {
            PyErr_SetString(PyExc_ValueError, "datetime_mode is not supportted for subinterpreter");
            return false;
        }
        if (PyLong_Check(arg)) {
            long mode = PyLong_AsLong(arg);
...
struct PyHandler {
    ...
    PyHandler(rapidjson_state* state,
              PyObject* decoder,
              PyObject* hook,
              unsigned dm,
              unsigned um,
          ...
          numberMode(nm)
        {
            ...
            if (decoder != NULL) {
                assert(!objectHook);
                if (PyObject_HasAttr(decoder, state->start_object_name)) {
                    decoderStartObject = PyObject_GetAttr(decoder, moduleState->start_object_name);
                }
                if (PyObject_HasAttr(decoder, state->end_object_name)) {
                    decoderEndObject = PyObject_GetAttr(decoder, moduleState->end_object_name);
                }
                if (PyObject_HasAttr(decoder, state->end_array_name)) {
                    decoderEndArray = PyObject_GetAttr(decoder, moduleState->end_array_name);
                }
                if (PyObject_HasAttr(decoder, state->string_name)) {
                    decoderString = PyObject_GetAttr(decoder, moduleState->string_name);
                }
            }
    ...
    bool NaN() {
        ...
        if (numberMode & NM_DECIMAL) {
            value = PyObject_CallFunctionObjArgs(moduleState->decimal_type, moduleState->nan_string_value, NULL);
        } else {
            value = PyFloat_FromString(moduleState->nan_string_value);
        }
    ...
    bool Infinity(bool minus) {
        ...
        PyObject* value;
        if (numberMode & NM_DECIMAL) {
            value = PyObject_CallFunctionObjArgs(moduleState->decimal_type,
                                                 minus
                                                 ? moduleState->minus_inf_string_value
                                                 : moduleState->plus_inf_string_value, NULL);
        } else {
            value = PyFloat_FromString(minus
                                       ? moduleState->minus_inf_string_value
                                       : moduleState->plus_inf_string_value);
        }
    ...
    bool RawNumber(const char* str, SizeType length, bool copy) {
        ...
        if (isFloat) {
            ...
            if (numberMode & NM_DECIMAL) {
                ...
                if (pystr == NULL)
                    return false;
                value = PyObject_CallFunctionObjArgs(moduleState->decimal_type, pystr, NULL);
                Py_DECREF(pystr);
            } else {
        ...
        if ((datetimeMode & DM_NAIVE_IS_UTC || isZ) && !hasOffset) {
            if (hasDate) {
                value = PyDateTimeAPI->DateTime_FromDateAndTime(
                    year, month, day, hours, mins, secs, usecs, moduleState->timezone_utc,
                    PyDateTimeAPI->DateTimeType);
            } else {
                value = PyDateTimeAPI->Time_FromTime(
                    hours, mins, secs, usecs, moduleState->timezone_utc, PyDateTimeAPI->TimeType);
            }
            ...
            if (offset == NULL) {
                ...
                PyObject* tz = PyObject_CallFunctionObjArgs(moduleState->timezone_type, offset, NULL);
                Py_DECREF(offset);
                if (tz == NULL) {
                    value = NULL;
                } else {
                    if (hasDate) {
                        ...
                        if (value != NULL && datetimeMode & DM_SHIFT_TO_UTC) {
                            PyObject* asUTC = PyObject_CallMethodObjArgs(
                                value, moduleState->astimezone_name, moduleState->timezone_utc, NULL);
                            Py_DECREF(value);
                            if (asUTC == NULL) {
                                value = NULL;
                            } else {
...
#undef digit
    ...
    bool HandleUuid(const char* str, SizeType length) {
        ...
        PyObject* value = PyObject_CallFunctionObjArgs(moduleState->uuid_type, pystr, NULL);
        Py_DECREF(pystr);
...
loads(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    rapidjson_state* state = get_rapidjson_state(self);

    PyObject* result = do_decode(state, NULL, jsonStr, jsonStrLen, NULL, 0, objectHook,
...
load(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    if (!PyObject_HasAttr(jsonObject, state->read_name)) {
    ...
    return do_decode(state, NULL, NULL, 0, jsonObject, chunkSize, objectHook,
                     numberMode, datetimeMode, uuidMode, parseMode);
}
...
static PyType_Spec DecoderType_Spec = {
    .name = "rapidjson.Decoder",
    .basicsize = sizeof(DecoderObject),
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE,
    .slots = DecoderType_Slots,
};
...
static PyObject*
do_decode(rapidjson_state* state, PyObject* decoder, const char* jsonStr, Py_ssize_t jsonStrLen,
          ...
          unsigned parseMode)
{
    ...
    PyHandler handler(state, decoder, objectHook, datetimeMode, uuidMode, numberMode);
    Reader reader;
    ...
    if (jsonStr != NULL) {
        ...
        PyReadStreamWrapper sw(state, jsonStream, chunkSize);
    ...
    if (reader.HasParseError()) {
        ...
        if (PyErr_Occurred()) {
            ...
            PyErr_Format(state->decode_error, "Parse error at offset %zu: %s",
...
decoder_call(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    rapidjson_state* state = find_rapidjson_state_by_def(Py_TYPE(self));
    ...
    } else if (PyObject_HasAttr(jsonObject, state->read_name)) {
        jsonStr = NULL;
        jsonStrLen = 0;
    } else {
        PyErr_SetString(
    ...
    PyObject* result = do_decode(state, self, jsonStr, jsonStrLen, jsonObject, chunkSize, NULL,
...
#define RECURSE(v) dumps_internal(state, writer, v, defaultFn,          \
...
#define ASSERT_VALID_SIZE(l) do {                                       \
    ...
    if (object == Py_None) {
        ...
        writer->Bool(object == Py_True);
               ...
               && (is_decimal = PyObject_IsInstance(object, state->decimal_type))) {
        if (is_decimal == -1) {
            return false;
        }
        ...
        if (!(numberMode & NM_NAN)) {
            bool is_inf_or_nan;
            PyObject* is_inf = PyObject_CallMethodObjArgs(object, state->is_infinite_name,
                                                          NULL);
            ...
            if (!is_inf_or_nan) {
                PyObject* is_nan = PyObject_CallMethodObjArgs(object, state->is_nan_name,
                                                              NULL);
        ...
        if (!(datetimeMode & DM_IGNORE_TZ)
            && PyObject_HasAttr(object, state->utcoffset_name)) {
            PyObject* utcOffset = PyObject_CallMethodObjArgs(object,
                                                             state->utcoffset_name,
                                                             NULL);
            ...
            if (utcOffset == Py_None) {
                ...
                if (datetimeMode & DM_NAIVE_IS_UTC) {
                    if (PyDateTime_Check(object)) {
                        ...
                        asUTC = PyDateTimeAPI->DateTime_FromDateAndTime(
                            ...
                            state->timezone_utc, PyDateTimeAPI->DateTimeType);
                    } else {
                        ...
                        asUTC = PyDateTimeAPI->Time_FromTime(
                            ...
                            state->timezone_utc, PyDateTimeAPI->TimeType);
                    }
                ...
                if (datetimeMode & DM_SHIFT_TO_UTC) {
                    ...
                    if (PyObject_IsTrue(utcOffset)) {
                        asUTC = PyObject_CallMethodObjArgs(object, state->astimezone_name,
                                                           state->timezone_utc, NULL);
        ...
        if (datetime_mode_format(datetimeMode) == DM_ISO8601) {
            ...
            timestampObj = PyObject_CallMethodObjArgs(midnightObj, state->timestamp_name,
                                                      NULL);
            ...
            if (datetimeMode & DM_ONLY_SECONDS) {
               ...
               && PyObject_TypeCheck(object, (PyTypeObject*) state->uuid_type)) {
        PyObject* hexval;
        if (uuidMode == UM_CANONICAL)
            hexval = PyObject_Str(object);
        else
            hexval = PyObject_GetAttr(object, state->hex_name);
        if (hexval == NULL)
            return false;
        ...
        writer->EndArray();
    } else if (PyObject_TypeCheck(object, state->rawjson_type)) {
        const char* jsonStr;
        Py_ssize_t l;
...
dumps(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    return do_encode(state, value, defaultFn, ensureAscii ? true : false, writeMode, indentChar,
...
dump(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    return do_stream_encode(state, value, stream, chunkSize, defaultFn,
...
static PyType_Slot EncoderType_Slots[] = {
    ...
    {Py_tp_getset, (void*)encoder_props},
...
static PyType_Spec EncoderType_Spec = {
    .name = "rapidjson.Encoder",
    .basicsize = sizeof(EncoderObject),
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE,
    .slots = EncoderType_Slots,
};
...
static PyObject*
do_encode(rapidjson_state* state, PyObject* value, PyObject* defaultFn, bool ensureAscii, unsigned writeMode,
...
static PyObject*
do_stream_encode(rapidjson_state* state, PyObject* value, PyObject* stream, size_t chunkSize, PyObject* defaultFn,
                 ...
                 unsigned mappingMode)
{
    ...
    PyWriteStreamWrapper os(state, stream, chunkSize);
...
encoder_call(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    if (stream != NULL && stream != Py_None) {
        if (!PyObject_HasAttr(stream, state->write_name)) {
        ...
        if (PyObject_HasAttr(self, state->default_name)) {
            defaultFn = PyObject_GetAttr(self, state->default_name);
        }

        result = do_stream_encode(state, value, stream, chunkSize, defaultFn, e->ensureAscii,
        ...
        result = do_encode(state, value, defaultFn, e->ensureAscii, e->writeMode, e->indentChar,
...
static PyType_Slot ValidatorType_Slots[] = {
    {Py_tp_dealloc, (void*)validator_dealloc},
    {Py_tp_call, (void*)validator_call},
    {Py_tp_doc, (void*)validator_doc},
    {Py_tp_new, (void*)validator_new},
    {Py_tp_free, (void*)PyObject_Del},
    {0, 0},
};


static PyType_Spec ValidatorType_Spec = {
    .name = "rapidjson.Validator",
    .basicsize = sizeof(ValidatorObject),
    .flags = Py_TPFLAGS_DEFAULT,
    .slots = ValidatorType_Slots,
};
...
static PyObject* validator_call(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    rapidjson_state* state = get_rapidjson_state(PyType_GetModule(Py_TYPE(self)));

    if (error) {
        if (asUnicode != NULL)
            Py_DECREF(asUnicode);
        PyErr_SetString(state->decode_error, "Invalid JSON");
        return NULL;
    }
    ...
    if (!accept) {
        ...
        PyErr_SetObject(state->validation_error, error);
...
static PyObject* validator_new(PyTypeObject* type, PyObject* args, PyObject* kwargs)
{
    ...
    rapidjson_state* state = get_rapidjson_state(PyType_GetModule(type));
...
module_exec(PyObject* m)
{
    ...
    rapidjson_state* state = get_rapidjson_state(m);

    state->decoder_type = (PyTypeObject*)PyType_FromModuleAndSpec(m, &DecoderType_Spec, NULL);
    if (state->decoder_type == NULL || PyModule_AddType(m, state->decoder_type)) {
        Py_CLEAR(state->decoder_type);
        return -1;
    }
    
    state->encoder_type = (PyTypeObject*)PyType_FromModuleAndSpec(m, &EncoderType_Spec, NULL);
    if (state->encoder_type == NULL || PyModule_AddType(m, state->encoder_type)) {
        Py_CLEAR(state->encoder_type);
        return -1;
    }
    
    state->validator_type = (PyTypeObject*)PyType_FromModuleAndSpec(m, &ValidatorType_Spec, NULL);
    if (state->validator_type == NULL || PyModule_AddType(m, state->validator_type)) {
        Py_CLEAR(state->validator_type);
        return -1;
    }
    
    state->rawjson_type = (PyTypeObject*)PyType_FromModuleAndSpec(m, &RawJSONType_Spec, NULL);
    if (state->rawjson_type == NULL || PyModule_AddType(m, state->rawjson_type)) {
        Py_CLEAR(state->rawjson_type);
        return -1;
    }
    ...
    state->uuid_type = PyObject_GetAttrString(uuidModule, "UUID");
    Py_DECREF(uuidModule);

    if (state->uuid_type == NULL)
        return -1;

    state->astimezone_name = PyUnicode_InternFromString("astimezone");
    if (state->astimezone_name == NULL)
        return -1;

    state->hex_name = PyUnicode_InternFromString("hex");
    if (state->hex_name == NULL)
        return -1;

    state->timestamp_name = PyUnicode_InternFromString("timestamp");
    if (state->timestamp_name == NULL)
        return -1;

    state->total_seconds_name = PyUnicode_InternFromString("total_seconds");
    if (state->total_seconds_name == NULL)
        return -1;

    state->utcoffset_name = PyUnicode_InternFromString("utcoffset");
    if (state->utcoffset_name == NULL)
        return -1;

    state->is_infinite_name = PyUnicode_InternFromString("is_infinite");
    if (state->is_infinite_name == NULL)
        return -1;

    state->is_nan_name = PyUnicode_InternFromString("is_nan");
    if (state->is_infinite_name == NULL)
        return -1;

    state->minus_inf_string_value = PyUnicode_InternFromString("-Infinity");
    if (state->minus_inf_string_value == NULL)
        return -1;

    state->nan_string_value = PyUnicode_InternFromString("nan");
    if (state->nan_string_value == NULL)
        return -1;

    state->plus_inf_string_value = PyUnicode_InternFromString("+Infinity");
    if (state->plus_inf_string_value == NULL)
        return -1;

    state->start_object_name = PyUnicode_InternFromString("start_object");
    if (state->start_object_name == NULL)
        return -1;

    state->end_object_name = PyUnicode_InternFromString("end_object");
    if (state->end_object_name == NULL)
        return -1;

    state->default_name = PyUnicode_InternFromString("default");
    if (state->default_name == NULL)
        return -1;

    state->end_array_name = PyUnicode_InternFromString("end_array");
    if (state->end_array_name == NULL)
        return -1;

    state->string_name = PyUnicode_InternFromString("string");
    if (state->string_name == NULL)
        return -1;

    state->read_name = PyUnicode_InternFromString("read");
    if (state->read_name == NULL)
        return -1;

    state->write_name = PyUnicode_InternFromString("write");
    if (state->write_name == NULL)
        return -1;

    state->encoding_name = PyUnicode_InternFromString("encoding");
    if (state->encoding_name == NULL)
        return -1;
...
#define XSTRINGIFY(x) #x
    ...
    state->validation_error = PyErr_NewException("rapidjson.ValidationError",
                                                 PyExc_ValueError, NULL);
    if (state->validation_error == NULL)
        return -1;
    Py_INCREF(state->validation_error);
    if (PyModule_AddObject(m, "ValidationError", state->validation_error) < 0) {
        Py_DECREF(state->validation_error);
        return -1;
    }

    state->decode_error = PyErr_NewException("rapidjson.JSONDecodeError",
                                             PyExc_ValueError, NULL);
    if (state->decode_error == NULL)
        return -1;
    Py_INCREF(state->decode_error);
    if (PyModule_AddObject(m, "JSONDecodeError", state->decode_error) < 0) {
        Py_DECREF(state->decode_error);
        return -1;
    }
...
static int
rapidjson_traverse(PyObject* module, visitproc visit, void *arg)
{
    rapidjson_state* state = get_rapidjson_state(module);

    Py_VISIT(state->decimal_type);
    Py_VISIT(state->timezone_type);
    Py_VISIT(state->timezone_utc);
    Py_VISIT(state->uuid_type);
    Py_VISIT(state->validation_error);
    Py_VISIT(state->decode_error);

    Py_VISIT(state->decoder_type);
    Py_VISIT(state->encoder_type);
    Py_VISIT(state->validator_type);
    Py_VISIT(state->rawjson_type);

    return 0;
}

static int
rapidjson_clear(PyObject* module)
{
    rapidjson_state* state = get_rapidjson_state(module);

    Py_CLEAR(state->decimal_type);
    Py_CLEAR(state->timezone_type);
    Py_CLEAR(state->timezone_utc);
    Py_CLEAR(state->uuid_type);
    Py_CLEAR(state->validation_error);
    Py_CLEAR(state->decode_error);
    ...
    Py_CLEAR(state->astimezone_name);
    Py_CLEAR(state->hex_name);
    Py_CLEAR(state->timestamp_name);
    Py_CLEAR(state->total_seconds_name);
    Py_CLEAR(state->utcoffset_name);
    Py_CLEAR(state->is_infinite_name);
    Py_CLEAR(state->is_nan_name);
    Py_CLEAR(state->start_object_name);
    Py_CLEAR(state->end_object_name);
    Py_CLEAR(state->default_name);
    Py_CLEAR(state->end_array_name);
    Py_CLEAR(state->string_name);
    Py_CLEAR(state->read_name);
    Py_CLEAR(state->write_name);
    Py_CLEAR(state->encoding_name);

    Py_CLEAR(state->minus_inf_string_value);
    Py_CLEAR(state->nan_string_value);
    Py_CLEAR(state->plus_inf_string_value);

    return 0;
}

static void
rapidjson_free(void* module)
{
    rapidjson_clear((PyObject *)module);
}
...
static PyModuleDef module = {
    ...
    sizeof(rapidjson_state),    /* m_size */
    functions,                  /* m_methods */
    slots,                      /* m_slots */
    rapidjson_traverse,         /* m_traverse */
    rapidjson_clear,            /* m_clear */
    rapidjson_free              /* m_free */
};

static rapidjson_state*
find_rapidjson_state_by_def(PyTypeObject* type) {
    PyObject* rapidjson_module = PyType_GetModuleByDef(type, &module);
    assert(rapidjson_module != NULL);
    return get_rapidjson_state(rapidjson_module);
}
