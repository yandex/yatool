...
void TModule::FinalizeConfig(ui32 id, const TModuleConf& conf) {
    ...
    if (conf.HasSemantics && !conf.CmdIgnore.empty()) {
        ...
        auto compiled = dummyCommandStore.Compile(conf.CmdIgnore, Vars, Vars, EOutputAccountingMode::Module);
        auto ignore = TCommands::SimpleCommandSequenceWriter()
            .Write(dummyCommandStore, compiled.Expression, Vars, dummyCmdInfo)
            .Extract();
