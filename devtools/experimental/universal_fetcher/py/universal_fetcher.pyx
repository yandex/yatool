...
cdef extern from "devtools/experimental/universal_fetcher/registry/registry.h" namespace "NUniversalFetcher" nogil:
    ...
    cdef cppclass TUniversalFetcher:
        ...
        THolder[TResult] DownloadToFile(const TString&, const TString&) nogil
        THolder[TResult] DownloadToDir(const TString&, const TString&) nogil

cdef extern from "devtools/experimental/universal_fetcher/py/helpers/helpers.h" namespace "NUniversalFetcher":
...
cdef class UniversalFetcher:
    ...
    def download(self, url, dst_path, filename=None):
        ...
        with nogil:
            if filename is not None:
                c_result = self._impl.Get().DownloadToFile(c_url, c_dst_path)
            else:
                c_result = self._impl.Get().DownloadToDir(c_url, c_dst_path)
