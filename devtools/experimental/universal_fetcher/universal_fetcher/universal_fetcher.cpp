...
namespace NUniversalFetcher {
    ...
    TUniversalFetcher::TResultPtr TUniversalFetcher::DownloadToDir(const TString& uri, const TString& dir) noexcept {
        return Download(uri, TDstPath::FromDirPath(dir));
    }

    TUniversalFetcher::TResultPtr TUniversalFetcher::DownloadToFile(const TString& uri, const TString& file) noexcept {
        return Download(uri, TDstPath::FromFilePath(file));
    }

    TUniversalFetcher::TResultPtr TUniversalFetcher::Download(const TString& uri, const TDstPath& path) noexcept {
        ...
        policy.Execute([&]() {
            ...
            try {
                attempt.Result = f.Fetcher->Fetch(uri, path);
            } catch (...) {
                attempt.Result.Status = EFetchStatus::InternalError;
                attempt.Result.Error = CurrentExceptionMessage();
            }
